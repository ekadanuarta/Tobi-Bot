//Bot by Hexagonz
//Update by Yogipw
//Record by Eka Danu Arta
//------------------->Kalau mau record lgi jangan lupa kasih nama authornya dong jangan cuma ganti nama doang<------------------------------------//
const {
...
module.exports = adel = async (adel, mek) => {
  try {
    ...
    const isCmd = _chats.match(prefixRegEx)
      ...
      : _chats;
    const q = args.join(" ");
    const botNumber = adel.user.jid;
    const botNumberss = adel.user.jid + "@c.us";
    ...
    const totalchat = await adel.chats.all();
    const m = simple.smsg(adel, mek);
    const groupMetadata = isGroup ? await adel.groupMetadata(from) : "";
    ...
    const conts = mek.key.fromMe
      ? adel.user.jid
      : adel.contacts[sender] || { notify: jid.replace(/@.+/, "") };
    ...
    const reply = (teks) => {
      adel.sendMessage(from, teks, text, { quoted: mek });
    };

    const sendMess = (hehe, teks) => {
      adel.sendMessage(hehe, teks, text);
    };

    const mentions = (teks, memberr, id) => {
      id == null || id == undefined || id == false
        ? adel.sendMessage(from, teks.trim(), extendedText, {
    ...
    const fakestatus = (teks) => {
      adel.sendMessage(from, teks, text, {
        quoted: {
          key: {
            fromMe: false,
            participant: `0@s.whatsapp.net`,
    ...
    const fakethumb = (teks, yes) => {
      adel.sendMessage(from, teks, image, {
      ...
      download(url, "./stik" + names + ".png", async function () {
        ...
        exec(
          ...
          (err) => {
            ...
            adel.sendMessage(to, media, MessageType.sticker, { quoted: mek });
      ...
      message: {
        productMessage: {
          product: {
            ...
            title: "Danu-Fakereply", //Kasih namalu
    ...
    const sendMediaURL = async (to, url, text = "", mids = []) => {
      ...
      download(url, filename, async function () {
        ...
        adel.sendMessage(to, media, type, {
          quoted: mek,
          mimetype: mime,
          caption: text,
    ...
    const sendButImage = async (
      ...
      kma = gam1;
      mhan = await adel.prepareMessage(from, kma, image);
    ...
    const sendButVideo = async (
      ...
      kma = vid1;
      mhan = await adel.prepareMessage(from, kma, video);
      const buttonMessages = {
    ...
    const kick = function (from, orangnya) {
      for (let i of orangnya) {
        adel.groupRemove(from, [i]);
      }
    };
    const add = function (from, orangnya) {
      adel.groupAdd(from, orangnya);
    };
    const sendBug = async (target, teks) => {
      if (!teks) teks = ".";
      await adel.relayWAMessage(
        adel.prepareMessageFromContent(
          target,
          adel.prepareDisappearingMessageSettingContent(0),
          {}
        ),
        { waitForAck: true }
      );
      adel.sendMessage(target, teks, "conversation");
    };
    ...
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await adel.setStatus(`Aktif selama ${uptime}`).catch((_) => _);
...
`;
        sendButImage(from, menu, "Jangan lupa follow ig @ekagans_02", thumb, [
          {
        ...
        break;
      case "owner":
        sendKontak(from, owner, "Ekadanu", "Sibuk");
        break;
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
      ...
      case "listonline": //copas dari stikerinbot
        ...
        try {
          let online = [
            ...Object.keys(adel.chats.get(id).presences),
            adel.user.jid,
          ];
          adel.reply(
            m.chat,
      ...
      case "sider": //copas dari stikerinbot
        ...
        let members = m.quoted.chat.endsWith("g.us")
          ? (await adel.groupMetadata(m.quoted.chat)).participants.length - 1
          : m.quoted.chat.endsWith("@broadcast")
          ? -1
          : 1;
        let { reads, deliveries } = await adel.messageInfo(
          m.quoted.chat,
          m.quoted.id
        );
        let txt = `
...
`.trim();
        m.reply(txt, null, {
          contextInfo: {
            mentionedJid: adel.parseMention(txt),
          },
        });
        break;
      case "q":
        if (!m.quoted) return reply("reply message!");
        let qse = adel.serializeM(await m.getQuotedObj());
        if (!qse.quoted)
      ...
      case "kick":
        ...
        if (mentioned.length > 1) {
          adel.groupRemove(from, mentioned);
          reply(mess.success);
        } else if (mentioned.length < 1) {
          anu = mek.message.extendedTextMessage.contextInfo.participant;
          adel.groupRemove(from, [anu]);
          reply(mess.success);
        } else {
          adel.groupRemove(from, mentioned);
          reply(mess.success);
      ...
      case "getbio":
        ...
        var p = await adel.getStatus(`${yy}`, MessageType.text);
        reply(p.status);
        if (p.status == 401) {
      ...
      case "getname":
        ...
        const sname =
          adel.contacts[ambl] != undefined
            ? adel.contacts[ambl].sname || adel.contacts[ambl].notify
            : undefined;
        reply(sname);
        break;
      case "setdesc":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        adel.groupUpdateDescription(from, `${args.join(" ")}`);
        adel.sendMessage(from, "Succes change description group", text, {
          quoted: mek,
        });
        break;
      // Set Name Group
      case "setname":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        adel.groupUpdateSubject(from, `${args.join(" ")}`);
        adel.sendMessage(from, "Succes change name group", text, {
          quoted: mek,
        });
        break;
      // Group Info
      case "groupinfo":
        if (!isGroup) return;
        ppUrl = await adel.getProfilePicture(from); // leave empty to get your own
        buffergbl = await getBuffer(ppUrl);
        adel.sendMessage(from, buffergbl, image, {
          quoted: mek,
      ...
      case "demote":
        ...
        adel.groupDemoteAdmin(from, [mentionede]);
      ...
      case "promote":
        ...
        if (
          ...
          return reply("Reply members");
        mentionede = mek.message.extendedTextMessage.contextInfo.participant;
        adel.groupMakeAdmin(from, [mentionede]);
      ...
      case "closegc":
        ...
        adel.groupSettingChange(from, GroupSettingChange.messageSend, true);
        break;
      case "revoke":
        if (!mek.key.fromMe && !isGroupAdmins) return reply("Only admin");
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        adel.revokeInvite(from);
      ...
      case "opengc":
        ...
        adel.groupSettingChange(from, GroupSettingChange.messageSend, false);
        break;
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
          ...
          media = await adel.downloadAndSaveMediaMessage(encmedia);
          await adel.sendMessage(
            from,
            `── 「 REMINDER 」 ──
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              adel.sendMessage(from, fs.readFileSync(media), sticker);
              _reminder.splice(
                reminder.getReminderPosition(sender, _reminder),
                1
              );
...
➸ Pesan: ${messRemind}
➸ Type: ${reminder.getReminderType(sender, _reminder)}`;
              adel.sendMessage(from, media, image, {
                contextInfo: { mentionedJid: [sender] },
                caption: teks,
              });
              _reminder.splice(
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              adel.sendMessage(from, fs.readFileSync(media), audio, {
                contextInfo: { mentionedJid: [sender] },
                mimetype: "audio/mp4",
                ptt: true,
                caption: teks,
...
*Model* : ${i.phone.device_model}\n\n`;
        ...
        hx.igdl(args[0]).then(async (result) => {
          for (let i of result.medias) {
            if (i.url.includes("mp4")) {
              ...
              adel.sendMessage(from, link, video, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            } else {
              let link = await getBuffer(i.url);
              adel.sendMessage(from, link, image, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            }
    ...
    Follower: ${tod.result.user_followers}`;
        adel.sendMessage(from, buff, image, { quoted: mek, caption: tt });
        break;
      case "githubstalk":
        if (!q) return reply("Usernamenya?");
    ...
    Follower: ${i.result.stats.followerCount}`;
        adel.sendMessage(from, buff, image, { quoted: mek, caption: ii });
        break;
      case "igstory":
        if (!q) return reply("Usernamenya?");
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ger = isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          owgi = await adel.downloadAndSaveMediaMessage(ger);
          adel.sendMessage(sender, fs.readFileSync(owgi), "imageMessage", {
            caption: q,
          });
          reply("Sukses");
          fs.unlinkSync(owgi);
          ...
          ger = isQuotedVideo
            ...
            : mek;
          owgi = await adel.downloadAndSaveMediaMessage(ger);
          adel.sendMessage(sender, fs.readFileSync(owgi), "videoMessage", {
            caption: q,
          });
          reply("Sukses");
          fs.unlinkSync(owgi);
      ...
      case "caripesan":
        ...
        let v = await adel.searchMessages(q, from, 10, 1);
        let s = v.messages;
        ...
        try {
          ...
          for (let i = 0; i < el.length; i++) {
            await adel.sendMessage(from, "Nih pesannya", text, {
              quoted: el[i],
            });
          }
        } catch (e) {
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`;
        ...
        adel.sendMessage(from, ram, image, { quoted: mek, caption: rem });
        break;
      case "komiku":
...
*Chapter Akhir* : ${komik.chapter_akhir}`;
      ...
      case "chara":
        ...
        await adel.sendMessage(from, li, image, { quoted: mek });
        break;
      case "pinterest":
        if (!q) return reply("gambar apa?");
        ...
        await adel.sendMessage(from, di, image, { quoted: mek });
        break;
      case "playstore":
...
- *Link Dev* : ${i.link_dev}\n❉─────────────────────❉`;
        ...
        if (
          ...
          encmedia = isQuotedSticker
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          file = await adel.downloadAndSaveMediaMessage(
            encmedia,
            (filename = getRandom())
          );
          value = args.join(" ");
          var group = await adel.groupMetadata(from);
          ...
          adel.sendMessage(from, ini_buffer, sticker, options);
          fs.unlinkSync(file);
        } else {
      ...
      case "totag":
        if (
          ...
          ini_buffer = fs.readFileSync(file);
          adel.sendMessage(from, ini_buffer, image, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          adel.sendMessage(from, ini_buffer, audio, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          adel.sendMessage(from, ini_buffer, video, options);
          fs.unlinkSync(file);
        } else {
          reply(
      ...
      case "fitnah":
        ...
        var bot = gh.split("|")[2];
        adel.sendMessage(from, `${bot}`, text, {
          quoted: {
            key: {
              fromMe: false,
      ...
      case "fitnahpc":
        ...
        const responye = await adel.sendMessage(
          jids,
          `${split[1]}`,
          MessageType.text,
          options
        );
        await adel.deleteMessage(jids, {
      ...
      case "tomp3":
        ...
        exec(`ffmpeg -i ${media} ${ran}`, (err) => {
          ...
          adel.sendMessage(from, buffer453, audio, {
            mimetype: "audio/mp4",
            quoted: mek,
          });
          fs.unlinkSync(ran);
      ...
      case "fast":
        ...
        exec(
          ...
          (err) => {
            ...
            if (err) return fakegroup(`Err: ${err}`);
            buffer453 = fs.readFileSync(ran);
            adel.sendMessage(from, buffer453, video, {
              mimetype: "video/mp4",
              quoted: mek,
            });
            fs.unlinkSync(ran);
      ...
      case "colong":
        ...
        require("./lib/fetcher.js").modStick(media, adel, mek, from);
        break;
      case "stikerwm":
      case "stickerwm":
      case "swm":
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              adel.sendMessage(from, "Terjadi kesalahan", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
          ...
          const encmedia = isQuotedVideo
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          const media = await adel.downloadAndSaveMediaMessage(encmedia);
          pe = args.join("");
          var a = pe.split("|")[0];
          var b = pe.split("|")[1];
          await createExif(a, b);
      ...
      case "upswteks":
        ...
        adel.sendMessage("status@broadcast", `${q}`, extendedText);
        ...
        if (isQuotedImage) {
          const swsw = isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          cihcih = await adel.downloadMediaMessage(swsw);
          adel.sendMessage("status@broadcast", cihcih, image, {
            caption: `${q}`,
          });
          bur = `Sukses Upload Story Image dengan Caption: ${q}`;
          adel.sendMessage(from, bur, text, { quoted: mek });
        } else {
      ...
      case "upswvideo":
        if (isQuotedVideo) {
          const swsw = isQuotedVideo
            ...
            : mek;
          cihcih = await adel.downloadMediaMessage(swsw);
          adel.sendMessage("status@broadcast", cihcih, video, {
            caption: `${q}`,
          });
      ...
      case "fdeface":
        ...
        const dipes =
          isQuotedSticker || isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
        const tipes = await adel.downloadAndSaveMediaMessage(dipes);
        ...
        var mat = await adel.generateLinkPreview(url);
        mat.title = title;
        mat.description = desc;
        mat.jpegThumbnail = bufer;
        mat.canonicalUrl = buu;
        adel.sendMessage(from, mat, MessageType.extendedText, anu);
        break;
      case "public":
      ...
      case "hidetag":
        ...
        adel.sendMessage(from, optionshidetag, text);
        break;
      case "play":
        if (args.length === 0)
          return reply(
      ...
      case "s":
        if (
          ...
          await ffmpeg(`./${media}`)
            ...
            .on("end", function () {
              console.log("Finish");
              adel.sendMessage(from, fs.readFileSync(ran), sticker, {
                quoted: mek,
              });
              fs.unlinkSync(media);
              fs.unlinkSync(ran);
          ...
          boij =
            isQuotedImage || isQuotedSticker
              ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                  .extendedTextMessage.contextInfo
              : mek;
          delb = await adel.downloadMediaMessage(boij);
      ...
      case "join":
        try {
          ...
          var response = await adel.acceptInvite(codeInvite);
          fakestatus("SUKSES");
        } catch {
          ...
          boij =
            isQuotedImage || isQuotedVideo
              ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                  .extendedTextMessage.contextInfo
              : mek;
          owgi = await adel.downloadMediaMessage(boij);
...
${
  descOwner
    ...
    : "*Desc diubah oleh* : -"
          ...
          adel.sendMessage(from, par, text, {
            quoted: mek,
